#!/usr/bin/env python3
"""Search one hybrid pure-PGS entry-switch family on the toy semiprime lane surface."""
...
import pgs_semiprime_backward_tighter_entry_law_search as tighter_base


DEFAULT_OUTPUT_DIR = ROOT / "output" / "semiprime_branch"
DEFAULT_MAX_N = 5_000
DEFAULT_MAX_STEPS = 24
SUMMARY_FILENAME = "pgs_semiprime_backward_hybrid_entry_law_search_summary.json"
LAW_ORDER = (
    "baseline_exact_shape_entry_switch",
    "hybrid_exact_shape_entry_switch",
)
HYBRID_SWITCH_MAP = {
    ("containing", True, True, 18, 4): ("previous", False, False, 26, 6),
    ("containing", True, True, 20, 2): ("containing", False, False, 20, 8),
    ("previous", False, False, 18, 16): ("previous", False, False, 18, 8),
    ("previous", False, False, 16, 10): ("previous", False, False, 16, 8),
}
BASELINE_LAW = "exact_shape_entry_switch"


def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser."""
    parser = argparse.ArgumentParser(
        description="Search one hybrid pure-PGS entry-switch family on the toy semiprime lane surface.",
    )
    parser.add_argument(
        "--max-n",
        type=int,
...
def select_entry_candidate(law_id: str, odd_candidates: list[dict[str, object]]) -> dict[str, object]:
    """Return the selected entry candidate for one hybrid law."""
    baseline = tighter_base.select_entry_candidate(BASELINE_LAW, odd_candidates)
    if law_id == "baseline_exact_shape_entry_switch":
        return baseline
    if law_id != "hybrid_exact_shape_entry_switch":
        raise ValueError(f"unknown law_id: {law_id}")

    target_shape = HYBRID_SWITCH_MAP.get(candidate_shape(baseline))
...
def run_case(law_id: str, modulus: int, max_steps: int) -> list[dict[str, object]]:
    """Run one deterministic hybrid entry trace."""
    if max_steps < 1:
    ...
    for step in range(1, max_steps + 1):
        candidates = [
            dict(candidate)
            for candidate in tighter_base.refined_base.cached_candidate_universe(current_anchor)
        ...
        if lane_factor is None:
            ...
            if odd_candidates:
                ...
                selected = tighter_base.refined_base.select_closure_candidate(prime_candidates, lane_factor)
...
def summarize_trace(trace_rows: list[dict[str, object]]) -> dict[str, object]:
    """Return the terminal payload for one hybrid trace."""
...
def run_search(max_n: int, max_steps: int) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Run the hybrid family over the toy corpus."""
    ...
    baseline_summary = json.loads(
        (DEFAULT_OUTPUT_DIR / tighter_base.SUMMARY_FILENAME).read_text(encoding="utf-8")
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the hybrid entry family and emit the summary JSON."""
    parser = build_parser()
