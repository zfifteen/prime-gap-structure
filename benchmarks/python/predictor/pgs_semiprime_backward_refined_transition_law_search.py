#!/usr/bin/env python3
"""Search a hand-designed refined pure-PGS transition family on the toy semiprime lane surface."""
...
SUMMARY_FILENAME = "pgs_semiprime_backward_refined_transition_law_search_summary.json"

REFINED_LAW_ORDER = (
    "baseline_small_gap_d4",
    "containing_if_prev_early_largegap_12",
    "containing_if_prev_early_largegap_14",
    "containing_if_prev_early_largegap_16",
    "containing_if_prev_early_largegap_18",
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Search a hand-designed refined pure-PGS transition family on the toy semiprime lane surface.",
    )
    parser.add_argument(
        "--max-n",
        type=int,
...
def parse_threshold(law_id: str) -> int | None:
    """Return the large-gap threshold for one refined law, if any."""
    if law_id == "baseline_small_gap_d4":
        return None
    return int(law_id.rsplit("_", 1)[1])


def baseline_entry_candidate(odd_candidates: list[dict[str, object]]) -> dict[str, object]:
    """Return the current best one-step baseline entry candidate."""
    return min(
        odd_candidates,
        key=lambda candidate: (
            lane_base.role_priority(str(candidate["role"])),
...
def containing_switch_candidate(odd_candidates: list[dict[str, object]]) -> dict[str, object] | None:
    """Return the best containing-gap odd carrier for the refined switch family."""
    containing_candidates = [candidate for candidate in odd_candidates if str(candidate["role"]) == "containing"]
...
def select_entry_candidate(law_id: str, odd_candidates: list[dict[str, object]]) -> dict[str, object]:
    """Return the selected refined entry candidate."""
    baseline = baseline_entry_candidate(odd_candidates)
    threshold = parse_threshold(law_id)
    if threshold is None:
        return baseline

    if (
        str(baseline["role"]) == "previous"
        and int(baseline["gap_width"]) >= threshold
        and bool(baseline["is_first_d4"])
    ):
        containing = containing_switch_candidate(odd_candidates)
        if containing is not None:
            return containing
    return baseline


def select_closure_candidate(prime_candidates: list[dict[str, object]], lane_factor: int | None) -> dict[str, object]:
    """Return the same-role closure nominee used by the current winning one-step family."""
    if lane_factor is None:
        return min(
            prime_candidates,
            key=lambda candidate: (
                0 if candidate["boundary_kind"] == "p_left" else 1,
                int(candidate["n"]),
            ),
        )
    return min(
        prime_candidates,
        key=lambda candidate: (
            0 if str(candidate["role"]) == "previous" else 1,
            0 if candidate["boundary_kind"] == "p_left" else 1,
            int(candidate["n"]),
        ),
    )
...
def run_case(law_id: str, modulus: int, max_steps: int) -> list[dict[str, object]]:
    """Run one deterministic refined transition trace."""
    if max_steps < 1:
    ...
    for step in range(1, max_steps + 1):
        ...
        if lane_factor is None:
            ...
            if odd_candidates:
                ...
                selected = select_closure_candidate(prime_candidates, lane_factor)
...
def summarize_trace(trace_rows: list[dict[str, object]]) -> dict[str, object]:
    """Return the terminal payload for one refined trace."""
...
def run_search(max_n: int, max_steps: int) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Run the fixed refined family over the toy corpus."""
    corpus = lane_base.generate_toy_corpus(max_n)
    trace_rows: list[dict[str, object]] = []
    traces_by_law: dict[str, list[dict[str, object]]] = {law_id: [] for law_id in REFINED_LAW_ORDER}

    for law_id in REFINED_LAW_ORDER:
...
def main(argv: list[str] | None = None) -> int:
    """Run the refined transition family and emit the summary JSON."""
    parser = build_parser()
