#!/usr/bin/env python3
"""Mine residual exact entry-switch opportunities under the current tighter backward lane law."""
...
DEFAULT_MAX_N = 5_000
SUMMARY_FILENAME = "pgs_semiprime_backward_residual_entry_pattern_miner_summary.json"
TARGET_TIGHTER_LAW = "exact_shape_entry_switch"


def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser."""
    parser = argparse.ArgumentParser(
        description="Mine residual exact entry-switch opportunities under the current tighter backward lane law.",
    )
    parser.add_argument(
        "--max-n",
        type=int,
...
def mine_residual_patterns(max_n: int) -> dict[str, object]:
    """Return the residual exact missed-entry switch surface for the current tighter law."""
    ...
    for modulus in corpus:
        ...
        selected = tighter_base.select_entry_candidate(TARGET_TIGHTER_LAW, odd_candidates)
    ...
    return {
        ...
        "target_tighter_law": TARGET_TIGHTER_LAW,
...
def main(argv: list[str] | None = None) -> int:
    """Run the residual missed-entry miner and emit its summary JSON."""
    parser = build_parser()
    args = parser.parse_args(argv)
    args.output_dir.mkdir(parents=True, exist_ok=True)

    summary = mine_residual_patterns(max_n=args.max_n)
