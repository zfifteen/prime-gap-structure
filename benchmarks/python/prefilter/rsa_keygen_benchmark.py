...
def confirm_prime_fixed_points(keypairs: Sequence[Dict[str, int]]) -> Dict[str, object]:
    """
    Confirm the prime fixed-point locus on final RSA primes via primality.

    Once a final keypair factor is confirmed prime, the Divisor Normalization Identity
    (DNI) `Z(n) = n^(1 - d(n)/2)` locks the invariant exactly at Z = 1.0.
    """
    confirmed_primes = 0
...
def build_rsa_report_section(title: str, results: Dict) -> List[str]:
    ...
    return [
        ...
        f"- Final keypair primes were confirmed by `sympy.isprime`; under the Divisor Normalization Identity (DNI) `Z(n) = n^(1 - d(n)/2)`, all `{fixed_points['fixed_point_count']}` confirmed factors remain exactly on the `Z = 1.0` fixed-point locus.",
        "",
