# Prime Gaps Are Generated by a Hierarchical Finite-State Model
...
triad the **Semiprime Wheel Attractor**. A deterministic lag-2 transition rule plus
...
finite-state model with three layers: a fast `14`-state core grammar, a
transition-rule layer, and a slower reset controller. The current surface therefore
...
3. Deterministic finite transition rules reproduce the measured transition
...
before the model is formalized. The object is already visible in the data.
...
This setup keeps the algorithm honest. It is trained and judged on the settled
...
correct, but what transition rule operated on that vocabulary.

The first successful closure beyond the plain rotor came from a lag-2
transition rule. On the sampled `256`-window surface, that model reduced pooled
...
This is the local model closure. On the present reduced surface, the local
...
eight-periodic. It is that the reduced model contains a small periodic law,
...
model was already tight, but the long walk still under-produced three-step
concentration.
...
aggressively. That split is evidence for a genuine hierarchy in the algorithm
rather than a defect in an otherwise flat model.

## 7. Frozen Model v1.0

The present model is frozen with three reference operating profiles.
...
1. The fast grammar reflects local wheel-sieve structure in the reduced state
   stream.
2. The transition rule reflects a slower periodic controller that biases which local
...
That endpoint does not weaken the model claim. It defines the next research
target.
...
There exists a hierarchical finite-state model on the reduced prime-gap type
surface whose core `14`-state grammar is scale-invariant above `10^7`, whose
dominant attractor is the Semiprime Wheel Attractor, and whose transition rule and
...
deterministic model family that reproduces the observed local and long-range
