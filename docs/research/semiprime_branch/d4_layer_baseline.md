...
- if the winner has `d(w)=4`, the tested surface shows no interior prime square
...
### B1 — Winner composition

What share of tested GWR winners are:
...
For `d=4` winner gaps, measure:

- offset of first interior `d=4`
- winner offset distribution
- whether first-`d=4` and winner offset always coincide on the committed surfaces
- conditional counts under square exclusion

### B3 — Dominance by scale

Measure the share of `d=4` winners across:
...
| `winner_d4_count` | number of gaps whose winner has `d=4` |
| `winner_d4_share` | `winner_d4_count / gap_count` |
| `winner_semiprime_count` | number of winners that are distinct semiprimes |
| `winner_prime_cube_count` | number of winners that are prime cubes |
| `winner_other_d4_count` | number of winners in `d=4` not covered above |
| `first_d4_match_count` | number of `d=4` winner gaps where first-`d=4` equals winner |
| `interior_square_violation_count` | gaps where a `d=4` winner occurs despite an interior prime square |
| `median_winner_offset` | median winner offset from left prime |
...
- the exact `2 x 10^7` surface reports `959,730` `d=4` winner gaps out of `1,163,198`
- that share is about `0.8251`
- the exact `2 x 10^7` surface reports `15` prime-cube winners inside the `d=4` winner class
- the current documented `d=4` winner surface reports `0` interior-square violations
- the current documented `d=4` winner surface reports exact first-`d=4` agreement on all tested surfaces
