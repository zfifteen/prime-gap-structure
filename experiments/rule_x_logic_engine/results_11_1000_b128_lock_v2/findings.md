...
It rejected `5072` candidate hypotheses, but it also rejected the true boundary
for `107 / 164` anchors.

The corrected oracle contrast is safe on this surface:

```text
lock only after the candidate boundary itself survives exact non-composite
screening
```

It rejected `2964` candidate hypotheses, rejected the true boundary `0` times,
...
rejects candidate boundaries whose proposed interior extends past the first
later lower-divisor composite.

It is too early. Many anchors begin with a trivial composite near `p`, then the
true gap later introduces a simpler carrier before the real boundary.
...
This rule waits until the candidate boundary itself survives exact
...
uses exact primality to avoid locking on a composite candidate boundary.
...
Of these, `44` transitions occurred before the actual boundary. These are the
...
after the candidate story has earned boundary-survivor status. Once that
