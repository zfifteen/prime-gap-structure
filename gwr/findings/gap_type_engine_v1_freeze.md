# GWR Gap-Type Model v1.0 Freeze

The strongest supported result is now concrete:

On the persistent `14`-state reduced gap-type surface, the Prime Gap
Generative Model is frozen as a deterministic three-layer model with a core
grammar, a transition rule, and a higher-divisor-triggered long-horizon controller.

This is an operational freeze, not a claim that every layer is finished in the
same sense. The local model is already very tight. The long-horizon layer is
...
The frozen model has three layers.

1. **Core grammar**: a persistent `14`-state reduced alphabet built on
   `open_family|d_bucket`.
2. **Transition-rule layer**: a lag-2 transition law with a small periodic correction.
...
The model is frozen with three reference operating profiles rather than one
...
window explanation, local comparison, and the main “model closes the observed
...
stationary `0.62` threshold while staying within the same deterministic model
family.
...
same controller setting. That is exactly what one expects if the model has a
fast layer and a slower re-entry law rather than a single flat rotor.

At the moment:

- the local model closes best with the nontriad-reset hybrid;
...
The model is operationally complete as a deterministic research instrument,
and the remaining extremal case is not whether the model exists, but what
...
model therefore keeps `8` as the working hybrid state while recording `11`
