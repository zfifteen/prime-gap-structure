# GWR Gap-Type Transition-Rule Findings
...
The transition rule probe aligns those regimes directly.

## Finite Transition-Rule Candidates

The new probe tests three deterministic models on the same persistent
`14`-state core:

1. `second_order_rotor`
   The existing baseline with no explicit transition rule.
2. `mod_cycle_scheduler`
   A simple `8`-state global counter.
3. `lag2_state_scheduler`
   A finite transition rule whose state is the reduced state two steps back. In
...
The missing transition rule is already visible as a finite local-memory layer, and on
...
the tested finite transition rules.

## Why The Modulo Transition Rule Matters
...
alphabet. A slow transition rule really is part of the phenomenon.
...
So even the best current transition rule is still too stationary. It improves the
...
- the core grammar plus a finite transition rule already explains the observed local
...
The transition rule is now visible in two independent ways.
...
Second, the `8`-state modulo transition rule says that a very small global rhythm can
...
2. a finite transition rule that controls local window state;
...
There exists a finite deterministic transition-rule layer above the persistent
`14`-state core, and when the model is scored on the same `256`-window regime
as the real `10^12..10^18` surface, that transition rule closes most of the observed
