# Selected integer Parity Next-Opening Probe Findings

When current selected integers are matched by the exact same current gap width and the
exact same current first-open offset, even selected integers feed the next gap back into
the odd-semiprime triad more often than odd selected integers do on the `10^12..10^15`
probe surface.

The measured combined matched next-triad shares are:

- even selected integers: `0.7023`
- odd selected integers: `0.6662`
...
and starting-offset imbalance between even-selected integer and odd-selected integer populations.
...
- `o4_odd_semiprime|d<=4` is modestly higher after even selected integers on the
  combined surface;
- `o6_odd_semiprime|d<=4` is modestly lower after even selected integers on the
  combined surface;
- `o2_odd_semiprime|d<=4` is nearly unchanged on the combined surface.

That means the strongest supported phrasing is not "even selected integers pick one exact
lane." The strongest supported phrasing is:

> even selected integers make next-gap return to the odd-semiprime triad more likely, and
...
- [selected integer parity next-opening probe script](../../benchmarks/python/predictor/gwr_winner_parity_next_opening_probe.py)
