...
SOURCE_PATH="$ROOT_DIR/src/BouncyCastleKeygenStrategyProbe.java"
OUTPUT_PATH="${1:-$ROOT_DIR/results/bcprov-jdk18on-1.83-source-r1rv83-rsa4096-strategy-probe-seed-byte-42.json}"
...
jar tf "$JAR_PATH" | grep -q '^org/bouncycastle/util/BigIntegers.class$'
jar tf "$JAR_PATH" | grep -q '^org/bouncycastle/math/Primes.class$'
...
JAVA_HOME="$BC_RUNTIME_JAVA_HOME" PATH="$BC_RUNTIME_JAVA_HOME/bin:$PATH" \
  java \
    ...
    BouncyCastleKeygenStrategyProbe \
    "$OUTPUT_PATH"
