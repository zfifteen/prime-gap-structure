...
BC_SOURCE_DIR="$ROOT_DIR/vendor/bc-java-r1rv83-rsa-upper-band-v1"
JAR_PATH="$BC_SOURCE_DIR/prov/build/libs/bcprov-jdk18on-1.83.jar"
SOURCE_PATH="$ROOT_DIR/src/BouncyCastleKeygenUpperBandV1Benchmark.java"
OUTPUT_PATH="$ROOT_DIR/results/bcprov-jdk18on-1.83-source-r1rv83-rsa-upper-band-v1-rsa4096-direct-core-seed-byte-42-runs-100.json"
...
if [ ! -d "$BC_SOURCE_DIR" ]; then
  echo "Missing modified Bouncy Castle source tree: $BC_SOURCE_DIR" >&2
  exit 1
fi
...
JAVA_HOME="$BC_RUNTIME_JAVA_HOME" PATH="$BC_RUNTIME_JAVA_HOME/bin:$PATH" \
  java \
    ...
    BouncyCastleKeygenUpperBandV1Benchmark \
    "$OUTPUT_PATH"
